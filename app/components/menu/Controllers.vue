...
<template>
  <div class="menu-controls__types text-txt-main flex items-center justify-between md:px-0 relative z-50 font-sans mb-2">
  
    <!-- MAKI -->
    <button 
      @click="$emit('update:makiOnly', !makiOnly)"
      class="menu-controls__type-btn flex justify-center items-center gap-2 md:gap-3 px-3 py-2 md:px-5 md:py-3 rounded-full border shadow-sm transition-all duration-200"
      :class="[
    ...
    >
      <img 
        ...
        class="menu-controls__icon dark:invert w-4 h-4 lg:w-5 lg:h-5 md:w-6 md:h-6"
        alt="" 
      />
  ...
  <div class="menu-controls__actions flex items-center justify-between pt-0 pb-3 md:px-0 relative z-50 font-sans">
    <!-- categorization btn -->
    <div class="menu-controls__category relative">
      <button 
        @click="isCategoryOpen = !isCategoryOpen"
        :class="[
        'menu-controls__btn menu-controls__btn--category flex items-center gap-1 md:gap-2 px-3 py-2 md:px-5 md:py-2.5 border border-txt-muted rounded-full transition-all active:scale-95 shadow-sm',
        activeCategory !== 'all'
    ...
    <button 
      ...
      class="menu-controls__btn menu-controls__btn--sale flex items-center gap-1 md:gap-2 px-3 py-2 md:px-5 md:py-2.5 rounded-full border border-txt-muted transition-all active:scale-95 shadow-sm"
    ...
    <div class="menu-controls__sort relative">
      <button 
        ...
        :class="[
          'menu-controls__btn menu-controls__btn--sort flex items-center gap-1 md:gap-2 px-3 py-2 md:px-5 md:py-2.5 border border-txt-muted rounded-full transition-all active:scale-95 shadow-sm',
